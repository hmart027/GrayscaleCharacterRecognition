...
public class SuperNeuronLayer extends NeuronLayerInterface {
	 ...
	 */
	public SuperNeuronLayer(int inSize, int outSize,  int upScaleBy, boolean keepLastWeights){
		if(upScaleBy<1) upScaleBy=1;
		reverseScaleFactor = 1.0f/(float)upScaleBy;
		this.inputLayerSize = inSize;
		this.outputLayerSize = outSize;
		...
		if(keepLastWeights){
			this.lastWeights 	=  new float[outputLayerSize*upScaleBy][inSize+1];
		}
	}
